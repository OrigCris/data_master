...
estruturados: separação pelo contrato da tabela (com quarentena), projeção com
renomeação e derivação de datas — evitando repetição entre URA/CALLS/PESQUISA.
"""
...
def validate_contract(
    ...
    *,
    ...
    O parse ocorreu na Bronze; aqui o **schema do contrato é a fonte única da verdade**:
    toda coluna declarada é obrigatória, então um evento é inválido quando qualquer uma
    delas está nula — ausente, incompatível ou vinda de um payload malformado (cujo parse
    na Bronze produziu nulos). O inválido não é descartado: vai para a DLQ com o
    **payload original** (auditável) e o motivo. `valid_events_df` mantém as colunas
    ...
    """
    contract_cols = contract.schema.fieldNames()
    valid_cond = F.lit(True)
    for c in contract_cols:
        valid_cond = valid_cond & F.col(c).isNotNull()

    valid_events_df = events_df.filter(valid_cond)

    missing_cols = F.array_compact(
        F.array(*[F.when(F.col(c).isNull(), F.lit(c)) for c in contract_cols])
    )
    event_id = F.sha2(
        F.concat_ws("|", F.lit(source), F.col(partition_col).cast("string"), F.col(offset_col).cast("string")),
        256,
    )
    quarantine_df = events_df.filter(~valid_cond).select(
        event_id.alias("event_id"),
        F.col(payload_col).cast("string").alias("payload"),
        F.concat(F.lit("missing_or_invalid: "), F.concat_ws(", ", missing_cols)).alias("error_reason"),
        F.lit(contract.version).alias("schema_version"),
        F.current_timestamp().alias("ingestion_ts"),
        F.lit(source).alias("source"),
    )
    return valid_events_df, quarantine_df
