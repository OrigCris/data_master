...
organizado em **módulos por domínio**. A identidade/segredos (Entra ID) da SPN
consumidora ficam no comando `dm setup-spn`, executado após o Bicep.
...
do ARM/Bicep. Essa etapa fica no comando `dm setup-spn`. O produtor não tem SPN: usa a
Managed Identity do Function App (papel *Data Sender* dado pelo Bicep). O Bicep cuida de
todos os recursos de plataforma, do RBAC e das app settings; o `dm setup-spn` cuida da
SPN consumidora e dos segredos.
